...
class MutualAttention(nn.Module):
    def __init__(self,
        ...
        # attention
        self.aa_guide_attention = MultiHeadAttention(latent_dim,
                                            latent_dim,
                                            latent_dim,
                                            latent_dim,
                                            num_heads=n_head,
        ...
        self.go_guide_attention = MultiHeadAttention(latent_dim,latent_dim,latent_dim,latent_dim,num_heads=n_head,dropout=dropout_rate,lambd=lambd)
        # gnn module
        self.gcn = GCN(latent_dim, latent_dim)
        self.go_transform_post = MLPLayer(int(2 * latent_dim), latent_dim)
        # classifier
        self.fc_out = MLPLayer(int(2*self.latent_dim), self.nb_classes)
    ...
    def forward(self, input_ids, lengths, labels, output_attention_weights=True):
        ...
        mean_batch = mean_batch.unsqueeze(1)
        # go embedder
        ...
        # output layer
        go_embedding = torch.cat((go_embedding, go_out), dim=1)
        go_embedding = self.go_transform_post(go_embedding)
        go_embedding = go_embedding.repeat((x.shape[0], 1, 1))
        # aa guild attention
        aa_guide_embedding, w_l = self.aa_guide_attention(mean_batch, go_embedding, go_embedding)
        # go guild attention
        go_guide_embedding, w_a = self.go_guide_attention(aa_guide_embedding, x, x, valid_lens=lengths+1)
        mutual_attention_embedding = torch.cat((aa_guide_embedding.squeeze(1), go_guide_embedding.squeeze(1)),dim=1)
        logits = self.fc_out(mutual_attention_embedding)
        outputs = logits
...
if __name__ == '__main__':
    ...
    adj = adj.cuda()
    model = MutualAttention(terms_embedding=terms_embedding,adj=adj,latent_dim=256,n_head=2,lambd=1)
