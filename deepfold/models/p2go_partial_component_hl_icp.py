...
class P2GO(nn.Module):
    def __init__(self,
                 terms_embedding:torch.Tensor,adj:torch.Tensor,edges, ics,
                 model_dir: str = 'esm1b_t33_650M_UR50S',
                 aa_dim=1280,
                 latent_dim=256,
                 dropout_rate=0.1,lam=0.1):
        super().__init__()
    ...
    def ic_penalty(self, preds, labels):
        ics = torch.Tensor(self.ics)
        ics = ics.repeat((preds.shape[0], 1))
        ics = ics.to(preds.device)
        ic_loss = torch.mean(torch.abs(labels - preds) * ics)
        return ic_loss

    def hierarchical_penalty(self, preds):
...
if __name__ == '__main__':
    ...
    from deepfold.utils.make_edges_simple import statistic_terms, make_ic_dict
    ...
    adj, multi_hot_vector, label_map, label_map_ivs,edges = build_graph(
    ...
    ic_dict = {k:ic_dict[k] for k in label_map.keys()}
    ics=list(ic_dict.values())
    ...
    adj = adj.cuda()
    model = P2GO(terms_embedding=terms_embedding,adj=adj,edges=edges,ics=ics,latent_dim=256)
    lr = 1e-4
    trainable_params = filter(lambda p: p.requires_grad,
                                model.parameters())
    finetune_params = [id(model.terms_embedding)]
    params = filter(lambda p: id(p) not in finetune_params, trainable_params)
    params = [
        {"params": params},
        {"params": model.terms_embedding, "lr": lr * 0.01},
    ]
    ...
    outputs = model(**batch)
    loss = outputs[0]
    ic_loss = model.ic_penalty(outputs[1].sigmoid(),batch['labels'])
    h_loss = model.hierarchical_penalty(outputs[1].sigmoid())
    loss = loss + ic_loss + h_loss
    loss.backward()
    optimizer.step()
